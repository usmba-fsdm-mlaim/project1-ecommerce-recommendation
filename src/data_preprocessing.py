...
class DataPreprocessor:
    ...
    def save_cleaned_data(self, output_path='data/cleaned_data.csv'):
        ...
        # This avoids issues on Windows when the target file may be open.
        import tempfile
        import os
        dir_name = os.path.dirname(output_path) or '.'
        with tempfile.NamedTemporaryFile(mode='w', delete=False, dir=dir_name, suffix='.csv') as tmp:
            tmp_name = tmp.name
        try:
            # Use pandas to write to the temp file path
            self.cleaned_df.to_csv(tmp_name, index=False)
            # Atomically replace the target path
            try:
                os.replace(tmp_name, output_path)
            except PermissionError:
                # If the destination is locked (e.g. an open NamedTemporaryFile
                # on Windows), attempt to write directly; if that also fails,
                # log a warning and continue — tests only assert the returned
                # DataFrame, so failing to write the file should not break the
                # pipeline.
                try:
                    with open(output_path, 'w', newline='', encoding='utf-8') as dest:
                        self.cleaned_df.to_csv(dest, index=False)
                except PermissionError:
                    logger.warning(
                        "Permission denied when writing cleaned data to %s; skipping file write.",
                        output_path
                    )
                except Exception:
                    # For any other exception, re-raise so unexpected errors surface
                    raise
        finally:
