...
class CollaborativeFilteringModel:
    ...
    def create_interaction_matrix(self, df: pd.DataFrame) -> csr_matrix:
        ...
        # If there are no interactions after filtering, raise to surface the issue
        if df_filtered.empty:
            raise ValueError("No interactions available after applying min_interactions filter")
    ...
    def predict_user_based(self, user_idx: int, top_k: int = 50) -> np.ndarray:
        ...
        # Get similar users (handle sparse matrix or ndarray)
        sim_row = self.user_similarity[user_idx]
        if hasattr(sim_row, "toarray"):
            user_sim = sim_row.toarray().ravel()
        else:
            user_sim = np.asarray(sim_row).ravel()
    ...
    def predict_item_based(self, user_idx: int, top_k: int = 50) -> np.ndarray:
        ...
        for item_idx in range(len(user_ratings)):
            if user_ratings[item_idx] == 0:  # Only predict for unrated items
                # handle sparse matrix or ndarray
                sim_row = self.item_similarity[item_idx]
                if hasattr(sim_row, "toarray"):
                    item_sim = sim_row.toarray().ravel()
                else:
                    item_sim = np.asarray(sim_row).ravel()
